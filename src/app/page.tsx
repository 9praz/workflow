...
function dueMeta(d: string | null) {
  ...
  return {
    ...
    overdue: diff < 0, soon: diff >= 0 && diff <= 2,
  }
}
function uid() { return Math.random().toString(36).slice(2, 10) }

// ─── DB helpers — all awaited, all fields explicit ────────────────────────────

async function dbUpdateTask(id: string, patch: Partial<Omit<Task,"id"|"assignees">>) {
...
function ConfirmDialog({ title, message, onConfirm, onCancel }: {
  ...
  const ok  = () => { setVis(false); setTimeout(onConfirm, 200) }
  const no  = () => { setVis(false); setTimeout(onCancel,  200) }
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center"
      style={{ background:`rgba(0,0,0,${vis?.3:0})`, backdropFilter:vis?"blur(4px)":"blur(0px)", transition:`background 200ms ${SM},backdrop-filter 200ms ${SM}` }}
      onClick={no}>
      <div onClick={e=>e.stopPropagation()}
        style={{ transform:vis?"scale(1)":"scale(0.95) translateY(12px)", opacity:vis?1:0, transition:`transform 280ms ${SP},opacity 220ms ${EZ}`, boxShadow:"0 20px 60px rgba(0,0,0,0.14),0 4px 16px rgba(0,0,0,0.06)" }}
        className="bg-white rounded-2xl border border-zinc-100 w-[340px] overflow-hidden">
        <div className="px-6 pt-6 pb-4 flex flex-col items-center text-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-red-50 flex items-center justify-center"
            style={{ animation:vis?`iconPop 400ms ${SP} 80ms both`:"none" }}>
        ...
        <div className="flex gap-2 px-6 pb-6">
          <button onClick={no}
            ...
            onMouseUp={e=>(e.currentTarget.style.transform="scale(1)")}>ยกเลิก</button>
          <button onClick={ok}
            className="flex-1 py-2.5 text-[13px] font-medium text-white bg-red-500 rounded-xl"
            style={{ transition:`background 120ms ${EZ}` }}
            onMouseDown={e=>(e.currentTarget.style.transform="scale(0.97)")}
            onMouseUp={e=>(e.currentTarget.style.transform="scale(1)")}>ลบ</button>
        </div>
      </div>
    </div>
  )
...
function Toast({ message, type="success", onDone }: {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => { setVis(false); setTimeout(onDone, 300) }, 2200)
    return () => clearTimeout(t)
  }, [])
  return (
    <div style={{ position:"fixed", bottom:24, left:"50%", zIndex:200, willChange:"transform,opacity",
      transform: vis?"translateX(-50%) translateY(0)":"translateX(-50%) translateY(16px)",
      opacity: vis?1:0, transition:`transform 300ms ${SP},opacity 250ms ${EZ}` }}>
      <div className={`flex items-center gap-2.5 px-4 py-3 rounded-xl text-[13px] font-medium ${type==="success"?"bg-zinc-900 text-white":"bg-red-500 text-white"}`}
...
export default function WorkflowPlanner() {
  ...
  const askConfirm = useCallback((title:string, message:string, onConfirm:()=>void) => setConfirm({title,message,onConfirm}), [])
  ...
  useEffect(() => {
    ...
    const debounced = () => { clearTimeout(timer); timer = setTimeout(fetchAll, 500) }
    const ch = supabase.channel("planner")
      .on("postgres_changes", {event:"*",schema:"public",table:"tasks"},          debounced)
      .on("postgres_changes", {event:"*",schema:"public",table:"task_assignees"}, debounced)
  ...
  // ── Save task field — awaited, explicit ───────────────────────────────────

  const saveTaskField = useCallback(async (id: string, patch: Partial<Omit<Task,"id"|"assignees">>) => {
    // 1. Update UI immediately
    setTasks(prev => prev.map(t => t.id === id ? {...t, ...patch} : t))
    // 2. Save to DB — await so realtime doesn't race
    const { error } = await dbUpdateTask(id, patch)
    if (error) {
  ...
  const createTask = useCallback(async (colId: Status, name: string) => {
    ...
    setAddingCol(null); setSelected(tempId)
    const { data, error } = await supabase.from("tasks")
      .insert({ name:name.trim(), description:"", status:colId, priority:"med", progress:0, tags:[], order_index:orderIdx })
      .select().single()
    if (data) {
      ...
      if (error) showToast("เพิ่มงานไม่สำเร็จ", "error")
    }
  }, [tasks, showToast])
  ...
  const deleteTask = useCallback((id: string, name: string) => {
    askConfirm("ลบงานนี้?", `"${name}" จะถูกลบถาวร`, async () => {
      setTasks(prev => prev.filter(t => t.id!==id))
      if (selected===id) setSelected(null)
      const { error } = await dbDeleteTask(id)
      if (error) { showToast("ลบไม่สำเร็จ","error"); fetchAll() }
      else showToast("ลบงานแล้ว")
    })
  ...
  const addMember = useCallback(async (name: string) => {
    ...
    if (data) { setMembers(prev => prev.map(m => m.id===tmp.id ? data : m)); showToast(`เพิ่ม ${name} แล้ว`) }
  }, [members, showToast])
  ...
  const renameMember = useCallback(async (id: string, name: string) => {
    ...
    setTasks(prev => prev.map(t => ({...t, assignees:t.assignees.map(a => a.id===id ? {...a,name:name.trim(),initials:name.trim().slice(0,2)} : a)})))
  ...
  const deleteMember = useCallback((id: string, name: string) => {
    askConfirm("ลบสมาชิก?", `"${name}" จะถูกลบและ unassign จากทุกงาน`, async () => {
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center gap-3 bg-[#f8f8f7]">
      <div className="w-8 h-8 rounded-xl bg-zinc-900 flex items-center justify-center"
        style={{ animation:`pulse 1.4s ${SM} infinite` }}>
        <Check className="w-4 h-4 text-white" />
      </div>
      <p className="text-[12px] text-zinc-400">กำลังโหลด...</p>
    </div>
  )
  ...
  return (
    <div className="h-screen flex flex-col bg-[#f8f8f7] overflow-hidden relative"
      ...
      {confirm && <ConfirmDialog {...confirm} onConfirm={()=>{confirm.onConfirm();setConfirm(null)}} onCancel={()=>setConfirm(null)} />}
        ...
        <div className="flex items-center gap-1.5">
          {members.map((m,i) => (
            <div key={m.id} style={{ animation:`fadeUp 300ms ${EZ} ${i*40}ms both` }}>
              <MemberPill member={m} active={filterMem===m.id} onClick={()=>setFilterMem(filterMem===m.id?null:m.id)} />
            </div>
          ))}
          <button onClick={()=>setMemberPanel(true)}
            ...
            style={{ transition:`border-color 150ms ${EZ},color 150ms ${EZ},transform 200ms ${SP}` }}
      ...
      {memberPanel && (
        <MemberPanel members={members} onAdd={addMember} onRename={renameMember}
      ...
      <div className="grid grid-cols-4 gap-2.5 px-5 py-3">
        {([
          ...
          <div key={s.label}
            className="bg-white border border-zinc-100 rounded-xl px-3.5 py-3"
            style={{ animation:`fadeUp 0.35s ${EZ} ${i*50}ms both`, transition:`transform 150ms ${SP},box-shadow 150ms ${EZ}` }}
        ...
        <div className="flex gap-3 flex-1 overflow-x-auto pb-1">
          {COLS.map(col => (
            <BoardColumn key={col.id} col={col}
              ...
              selectedId={selected} addingHere={addingCol===col.id}
              newInputRef={addingCol===col.id?newInputRef:undefined}
              dragState={dragState}
              onSelect={id=>setSelected(selected===id?null:id)}
              onAdd={()=>setAddingCol(col.id)} onCancelAdd={()=>setAddingCol(null)}
              onConfirmAdd={name=>createTask(col.id,name)}
              onDragStart={onDragStart} onDragEnter={onDragEnter}
              onDropCol={()=>onDropCol(col.id)} onDropCard={tid=>onDropCard(tid,col.id)}
              onDelete={deleteTask} onSave={saveTaskField} />
          ))}
        </div>
        ...
        <div className="w-[272px] shrink-0" style={{
          ...
          {selectedTask && (
            <DetailPanel task={selectedTask} members={members}
              onSave={saveTaskField} onToggleAssignee={toggleAssignee}
      ...
      <style>{`
        ...
        @keyframes iconPop {0%{transform:scale(0) rotate(-10deg)}60%{transform:scale(1.15)}100%{transform:scale(1)}}
...
const BoardColumn = React.memo(function BoardColumn({
  ...
  onDragStart, onDragEnter, onDropCol, onDropCard, onDelete, onSave,
}: {
  col:{id:Status;label:string;accent:string}; tasks:Task[]; selectedId:string|null
  addingHere:boolean; newInputRef?:React.RefObject<HTMLInputElement>
  dragState:{from:string|null;over:string|null}
  onSelect:(id:string)=>void; onAdd:()=>void; onCancelAdd:()=>void; onConfirmAdd:(n:string)=>void
  onDragStart:(id:string)=>void; onDragEnter:(id:string)=>void
  onDropCol:()=>void; onDropCard:(id:string)=>void
  onDelete:(id:string,name:string)=>void
  onSave:(id:string,patch:Partial<Omit<Task,"id"|"assignees">>)=>void
}) {
      ...
      <div className="flex flex-col gap-1.5 flex-1 overflow-y-auto min-h-[60px]">
        {tasks.map((t,i) => (
          <TaskCard key={t.id} task={t} col={col}
            selected={selectedId===t.id} isDragging={dragState.from===t.id} isDragOver={dragState.over===t.id} index={i}
            onSelect={()=>onSelect(t.id)} onDragStart={()=>onDragStart(t.id)}
            onDragEnter={()=>onDragEnter(t.id)} onDrop={()=>onDropCard(t.id)}
            onDelete={()=>onDelete(t.id,t.name)}
            onSaveName={name=>onSave(t.id,{name})} />
        ))}
        {isDropTarget && (
          <div className="h-14 rounded-xl border-2 border-dashed flex items-center justify-center"
            style={{borderColor:col.accent+"60",background:col.accent+"08",animation:`fadeUp 150ms ${EZ} both`}}>
        ...
        <div className="bg-white border border-zinc-200 rounded-xl p-2.5"
          ...
          <div className="flex gap-1.5">
            <button onClick={confirm}
              ...
              onMouseDown={e=>(e.currentTarget.style.transform="scale(0.97)")}
              onMouseUp={e=>(e.currentTarget.style.transform="scale(1)")}>เพิ่ม</button>
            <button onClick={cancel} className="px-2.5 py-1.5 text-zinc-400 border border-zinc-200 rounded-lg">
              <X className="w-3 h-3" />
            </button>
          </div>
        </div>
...
const TaskCard = React.memo(function TaskCard({
  ...
  onSelect, onDragStart, onDragEnter, onDrop, onDelete, onSaveName,
}: {
  ...
  onDelete:()=>void; onSaveName:(name:string)=>void
}) {
  ...
  const commit = () => { setEditing(false); if(val.trim()&&val!==task.name) onSaveName(val.trim()); else setVal(task.name) }

  return (
    <div draggable onDragStart={onDragStart} onDragEnter={onDragEnter}
      onDragOver={e=>e.preventDefault()} onDrop={e=>{e.stopPropagation();onDrop()}}
      onClick={onSelect}
      ...
      <div className="absolute left-0 top-3 bottom-3 w-[3px] rounded-full" style={{background:col.accent}} />
      <div className="flex items-start gap-1.5 pl-3 pr-1 mb-1.5">
        {editing ? (
          <input ref={ref} value={val} onChange={e=>setVal(e.target.value)} onBlur={commit}
            onKeyDown={e=>{if(e.key==="Enter")commit();if(e.key==="Escape"){setEditing(false);setVal(task.name)}}}
        ...
        <div className="flex items-center gap-0.5 shrink-0 opacity-0 group-hover:opacity-100"
          style={{transition:`opacity 150ms ${EZ}`}} onClick={e=>e.stopPropagation()}>
          <QuickBtn title="แก้ไขชื่อ" onClick={()=>setEditing(true)}><Pencil className="w-2.5 h-2.5"/></QuickBtn>
          <QuickBtn title="ลบงาน" danger onClick={onDelete}><Trash2 className="w-2.5 h-2.5"/></QuickBtn>
          <span className="w-4 h-4 flex items-center justify-center cursor-grab text-zinc-300">
            <GripVertical className="w-3 h-3"/>
          </span>
        </div>
      </div>
      {task.description && <p className="text-[10px] text-zinc-400 leading-relaxed pl-3 mb-1.5 line-clamp-1">{task.description}</p>}
      <div className="flex items-center gap-1 pl-3 flex-wrap">
        ...
        {task.tags.slice(0,1).map(tg=><span key={tg} className="text-[9px] px-1.5 py-0.5 rounded-md bg-zinc-100 text-zinc-500">{tg}</span>)}
        {due && (
          <span className={`flex items-center gap-0.5 text-[9px] px-1.5 py-0.5 rounded-md ml-auto ${due.overdue?"bg-red-50 text-red-500":due.soon?"bg-amber-50 text-amber-600":"bg-zinc-100 text-zinc-500"}`}>
      ...
      {task.status!=="todo" && (
        <div className="mt-2 pl-3">
          <div className="h-1 bg-zinc-100 rounded-full overflow-hidden">
            <div className="h-full rounded-full" style={{width:`${task.progress}%`,background:col.accent,transition:`width 500ms ${EZ}`}} />
          </div>
        </div>
      )}
    </div>
...
function QuickBtn({children,onClick,danger,title}:{children:React.ReactNode;onClick:()=>void;danger?:boolean;title?:string}) {
  return (
...
const MemberPill = React.memo(function MemberPill({member:m,active,onClick}:{member:Member;active:boolean;onClick:()=>void}) {
  return (
...
const DetailPanel = React.memo(function DetailPanel({
  task, members, onSave, onToggleAssignee, onDelete, onClose,
}: {
  ...
  // Local state — synced from task prop when task.id changes
  ...
  useEffect(() => {
    ...
    setShowDue(!!task.due_date)
  }, [task.id])

  // Flush text fields on blur
  const saveName = () => { if(name.trim()!==task.name) onSave(task.id,{name:name.trim()||task.name}) }
  const saveDesc = () => { if(desc!==task.description) onSave(task.id,{description:desc}) }
  const saveDue  = (val:string) => { setDue(val); onSave(task.id,{due_date:val||null}) }
    ...
    <div className="flex flex-col h-full bg-white border border-zinc-100 rounded-2xl overflow-hidden"
      ...
      <div className="flex items-start justify-between gap-2 px-4 pt-4 pb-3 border-b border-zinc-100">
        ...
        <button onClick={onClose}
          ...
          style={{transition:`transform 200ms ${SP}`}}
          ...
          <div className="grid grid-cols-3 gap-1">
            {COLS.map(c=>(
              <Seg key={c.id} active={task.status===c.id} accent={c.accent}
                onClick={()=>onSave(task.id,{status:c.id,progress:c.id==="done"?100:c.id==="todo"?0:task.progress})}>
                {c.label}
              </Seg>
            ))}
          </div>
        ...
        <FG label="ความสำคัญ">
          <div className="grid grid-cols-3 gap-1">
            {(["high","med","low"] as Priority[]).map(p=>{
              ...
              return (
                <button key={p} onClick={()=>onSave(task.id,{priority:p})}
                  className="flex items-center justify-center gap-1 py-1.5 rounded-lg text-[11px] font-medium border"
                  style={{background:act?cfg.color+"18":"transparent",borderColor:act?cfg.color+"50":"#f1f0ef",color:act?cfg.color:"#a1a1aa",transition:`background 160ms ${EZ},border-color 160ms ${EZ},color 160ms ${EZ},transform 120ms ${SP}`}}
                  onMouseDown={e=>(e.currentTarget.style.transform="scale(0.95)")}
                  onMouseUp={e=>(e.currentTarget.style.transform="scale(1)")}>
            ...
            {members.map(m=>{
              ...
              return (
                <button key={m.id} onClick={()=>onToggleAssignee(task.id,m.id)}
                  ...
                  style={{background:assigned?m.bg:"transparent",borderColor:assigned?m.color+"60":"#f1f0ef",color:assigned?m.color:"#a1a1aa",transition:`all 160ms ${EZ}`}}
                  onMouseDown={e=>(e.currentTarget.style.transform="scale(0.95)")}
                  onMouseUp={e=>(e.currentTarget.style.transform="scale(1)")}>
                  <span className="w-3.5 h-3.5 rounded-full flex items-center justify-center text-[7px] font-semibold" style={{background:m.bg,color:m.color}}>{m.initials}</span>
                  {m.name}
          ...
          {showDue ? (
            <div className="flex items-center gap-2">
              ...
              <input type="date" value={due} onChange={e=>saveDue(e.target.value)}
                ...
                style={{transition:`border-color 150ms ${EZ}`}} />
              <button onClick={()=>{setShowDue(false);saveDue("")}} className="text-zinc-400"
                style={{transition:`transform 200ms ${SP}`}}
                onMouseEnter={e=>(e.currentTarget.style.transform="rotate(90deg)")}
                onMouseLeave={e=>(e.currentTarget.style.transform="rotate(0deg)")}>
                <X className="w-3.5 h-3.5"/>
        ...
        {task.status!=="todo" && (
          <FG label={`ความคืบหน้า — ${task.progress}%`}>
            <input type="range" min="0" max="100" step="5" value={task.progress}
              onChange={e=>onSave(task.id,{progress:Number(e.target.value)})}
              className="w-full accent-zinc-900" />
            <div className="h-1 bg-zinc-100 rounded-full mt-1.5 overflow-hidden">
              <div className="h-full rounded-full" style={{width:`${task.progress}%`,background:col.accent,transition:`width 400ms ${EZ}`}} />
            </div>
          </FG>
        )}
        ...
        <FG label="หมวดหมู่">
          <div className="flex flex-wrap gap-1">
            {TAGS.map(tg=>{
              ...
              return (
                <button key={tg}
                  onClick={()=>onSave(task.id,{tags:act?task.tags.filter(t=>t!==tg):[...task.tags,tg]})}
                  className="text-[10px] px-2.5 py-1 rounded-full border"
                  style={{background:act?"#18181b":"transparent",color:act?"#fff":"#a1a1aa",borderColor:act?"#18181b":"#f1f0ef",transition:`all 160ms ${EZ},transform 120ms ${SP}`}}
      ...
      <div className="flex gap-2 px-4 py-3 border-t border-zinc-100">
        ...
        <button onClick={()=>onSave(task.id,{name,description:desc,due_date:due||null})}
...
function MemberPanel({members,onAdd,onRename,onDelete,onClose}:{
  members:Member[]; onAdd:(n:string)=>void; onRename:(id:string,n:string)=>void
  onDelete:(id:string,n:string)=>void; onClose:()=>void
}) {
  const [newName,  setNewName]  = useState("")
  ...
  useEffect(()=>{ requestAnimationFrame(()=>setVis(true)); newRef.current?.focus() },[])
  useEffect(()=>{ if(editId) editRef.current?.focus() },[editId])
  const commitEdit = () => { if(editId){onRename(editId,editVal);setEditId(null)} }
  const submitNew  = () => { if(newName.trim()){onAdd(newName.trim());setNewName("")} }
  const handleClose = () => { setVis(false); setTimeout(onClose,200) }

  return (
    <div className="absolute inset-0 z-50 flex items-start justify-end pt-14 pr-5"
      style={{background:`rgba(0,0,0,${vis?.15:0})`,backdropFilter:vis?"blur(3px)":"blur(0px)",transition:`background 200ms ${SM},backdrop-filter 200ms ${SM}`}}
      onClick={handleClose}>
      <div onClick={e=>e.stopPropagation()}
        className="w-[300px] bg-white rounded-2xl border border-zinc-100 overflow-hidden"
        style={{transform:vis?"translateY(0) scale(1)":"translateY(-10px) scale(0.97)",opacity:vis?1:0,transition:`transform 280ms ${SP},opacity 220ms ${EZ}`,boxShadow:"0 12px 40px rgba(0,0,0,0.10),0 2px 8px rgba(0,0,0,0.05)"}}>

        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100">
          <span className="text-[13px] font-semibold text-zinc-900">สมาชิกในทีม</span>
          <button onClick={handleClose} className="w-6 h-6 flex items-center justify-center rounded-lg text-zinc-400"
            style={{transition:`transform 200ms ${SP}`}}
            onMouseEnter={e=>(e.currentTarget.style.transform="rotate(90deg)")}
            onMouseLeave={e=>(e.currentTarget.style.transform="rotate(0deg)")}>
            <X className="w-3.5 h-3.5"/>
        ...
        <div className="max-h-[320px] overflow-y-auto">
          {members.map((m,i)=>(
            <div key={m.id}
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100" style={{transition:`opacity 120ms ${EZ}`}}>
                <button onClick={()=>{setEditId(m.id);setEditVal(m.name)}}
...
function Seg({active,accent,onClick,children}:{active:boolean;accent:string;onClick:()=>void;children:React.ReactNode}) {
  return (
    <button onClick={onClick} className="py-1.5 rounded-lg text-[10px] font-medium border"
      style={{background:active?accent:"transparent",color:active?"#fff":"#a1a1aa",borderColor:active?accent:"#f1f0ef",transition:`all 160ms ${EZ},transform 120ms ${SP}`}}
      onMouseDown={e=>(e.currentTarget.style.transform="scale(0.95)")}
      onMouseUp={e=>(e.currentTarget.style.transform="scale(1)")}>
      {children}
    </button>
